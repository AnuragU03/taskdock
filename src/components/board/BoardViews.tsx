      ...
      return (
        <div key={col.id} className="kanban-col">
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10, padding: '0 2px' }}>
            ...
            <span style={{ fontSize: 12, fontFamily: 'DM Mono, monospace', color: 'var(--t4)' }}>{ct.length}</span>
          </div>
          {ct.map(t => <TCard key={t.id} task={t} onClick={() => onClickTask(t.id)} compact />)}
          {ct.length === 0 && (
            <div style={{ padding: '18px', textAlign: 'center', color: 'var(--t4)', fontSize: 12, fontFamily: 'DM Mono, monospace', background: 'var(--bg1)', borderRadius: 12, border: `1px dashed var(--border)` }}>
              Empty
            </div>
          )}
        </div>
        ...
        return (
          <div key={t.id} className="card fu" onClick={() => onClickTask(t.id)} style={{ padding: 0, display: 'flex', overflow: 'hidden', height: 84, cursor: 'pointer' }}>
            <div style={{ width: 130, flexShrink: 0, background: 'var(--bg1)', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '10px 14px', borderRight: '1px solid var(--border)' }}>
              <CDSmall task={t} />
              <div style={{ fontSize: 12, fontFamily: 'DM Mono, monospace', color: 'var(--t4)', marginTop: 3 }}>{fmtS(t.dueAt)}</div>
            </div>
            ...
            <div style={{ flex: 1, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', gap: 5, marginBottom: 4, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {t.category && <span style={{ fontSize: 12, fontFamily: 'DM Mono, monospace', background: 'var(--bg3)', padding: '2px 5px', borderRadius: 3, color: 'var(--t4)' }}>{t.category}</span>}
                </div>
                <div style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 600, fontSize: 16, color: 'var(--t1)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', letterSpacing: '-.2px' }}>{t.title}</div>
