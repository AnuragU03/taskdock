        ...
        <div style={{ padding: '24px 28px', maxWidth: 620 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 8, marginBottom: 18 }}>
            {[
              ...
              { l: 'Assigned to', v: (task.assignee && task.status !== 'abandoned') ? <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 5 }}><Av user={task.assignee} sz={40} /><span style={{ fontSize: 15, color: 'var(--t1)' }}>{task.assignee.name}</span></div> : <span style={{ fontSize: 13, color: 'var(--t4)', marginTop: 5, display: 'block' }}>—</span> },
